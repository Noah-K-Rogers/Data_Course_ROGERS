...
ggplot(A_states,aes(y=Deaths,x=Last_Update))+
  labs(x="Date", y = "Total Deaths", title = "Cumulative Deaths over Time by State")+
...
ggplot(max_rates, aes(x= reorder(Province_State, -Maximum_Fatality_Ratio), y=Maximum_Fatality_Ratio))+
  labs(x="State", y="Maximum Fatality Ratio",title = "Death Ratios by State")+
  theme_minimal()+
...
death_over_time$Last_Update <- as.Date(death_over_time$Last_Update)
ggplot(death_over_time,aes(x=Last_Update,y = US_deaths))+
  geom_line(linewidth = 1.5)+
  theme_minimal()+
  labs(x="Date", y="Deaths", title = "Cumulative Deaths in USA")
